...
void init_motor_states(void) {
  ...
  if (!x_states[1]) {
    L6470_Run(X_AXIS_MOTOR_ID, L6470_DIR_FWD_ID, CURR_SPEED);
  } else if (!x_states[0]) {
  ...
  if (!y_states[1]) {
    ...
    L6470_Run(Y_AXIS_MOTOR_ID, L6470_DIR_REV_ID, CURR_SPEED);
    y_direction = 0;
  }
}
void process_motor_state(uint8_t L6470_Id, int switch_dir) {
  ...
  if (L6470_Id == X_AXIS_MOTOR_ID) {
    ...
    if (pin_state && switch_dir == x_direction && !x_states[switch_dir ^ 0x1]) {
      ...
      L6470_Run(X_AXIS_MOTOR_ID, switch_dir, CURR_SPEED);
    ...
    if (pin_state && switch_dir == y_direction && !y_states[switch_dir ^ 0x1]) {
      ...
      L6470_Run(Y_AXIS_MOTOR_ID, switch_dir, CURR_SPEED);
